## This script is for compiling stop-and-frisk data across 2014-2023.

library(tidyverse)
library(janitor)
library(lubridate)

# Creating a main filepath linked to the subfolder containing raw data
main_path <- paste0(getwd(), "/raw-data/")

# Loading 2023 data as a base
raw_sqf_2023 <- read_csv(paste0(main_path,"sqf-2023.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2023)
...
## Note: I chose not to iterate this because the processing 
## required for each year's data is different.

##### 2022 data #####
raw_sqf_2022 <- read_csv(paste0(main_path,"sqf-2022.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2022)
...
##### 2021 data #####
raw_sqf_2021 <- read_csv(paste0(main_path,"sqf-2021.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2021)
...
##### 2020 data #####
raw_sqf_2020 <- read_csv(paste0(main_path,"sqf-2020.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2020)
...
##### 2019 data #####
raw_sqf_2019 <- read_csv(paste0(main_path,"sqf-2019.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2019)
...
##### 2018 data #####
raw_sqf_2018 <- read_csv(paste0(main_path,"sqf-2018.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2018)
...
##### 2017 data #####
raw_sqf_2017 <- read_csv(paste0(main_path,"sqf-2017.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2017)
...
raw_sqf_2016 <- read_csv(paste0(main_path,"sqf-2016.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2016) %>% 
...
raw_sqf_2015 <- read_csv(paste0(main_path,"sqf-2015.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2015) %>% 
  rename_at(vars(one_of(old_names)), ~ new_names)
...
##### 2014 data #####
# 2014 data is similar to 2015/16, so the same renaming steps are required
raw_sqf_2014 <- read_csv(paste0(main_path,"sqf-2014.csv")) %>% 
  clean_names() %>% 
  mutate(year = 2014) %>% 
  rename_at(vars(one_of(old_names)), ~ new_names)
...
## 2 checks to make sure all data is cleaned properly
